...
public interface CommentRepository extends JpaRepository<Comment,Long> {
    @Query("select c from Comment c where c.posts.id=:postId and c.parent is null order by c.id desc")
    List<Comment> findAllDesc(Pageable page,@Param("postId") Long id);

    @Query("select c from Comment c where c.parent.id=:parentId order by c.id desc")
